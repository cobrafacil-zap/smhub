...
interface EditorialCalendarProps<E extends CalendarEntry> {
...
export function EditorialCalendar<E extends CalendarEntry>({
  ...
  const cells: CalendarCell<E>[] = useMemo(
    () => buildMonthCells<E>(year, month, entries),
    [year, month, entries]
  );
          ...
          >
            ...
            <div className="space-y-0.5">
              {cell.entries.slice(0, 3).map((e) => {
                // Render padrão: cor fixa por tipo (planejamento). renderChip
                // permite outro esquema (ex.: gravações por status).
                const c = renderChip
                  ? renderChip(e)
                  : {
                      label: `${ENTRY_TIPO_LABEL[e.tipo] ?? e.tipo}: ${e.titulo}`,
                      chip: (ENTRY_TIPO_COR[e.tipo] ?? ENTRY_TIPO_COR.post_feed).chip,
                    };
                return (
                  <div
                    key={e.id}
                    ...
                    title={e.titulo}
                  >
                    {renderChip ? c.label : (<>{c.label}{e.estilo ? <span className="opacity-70"> · {e.estilo}</span> : null}</>)}
                  </div>
                );
              })}
