...
interface Props {
  ...
  /** Handler custom de "aceitar/adicionar". Se ausente, usa o fluxo da agência
   *  (criarEntradaAction + planejamentoId). Usado na visão do cliente. */
  onAdicionar?: (data: DataComemorativa) => Promise<{ error?: string } | null | undefined>;
  /** Datas (YYYY-MM-DD) que já têm entrada no planejamento → exibidas como
   *  "Adicionado" em vez do botão de adicionar. */
  datasComEntrada?: string[];
  /** Texto do botão de aceitar/adicionar (default "Adicionar"). */
  aceitarLabel?: string;
}
 ...
 *  - Adicionada/aceita como entrada (cria um post no planejamento)
...
export function DatasComemorativasSugestoes({
  ...
  function adicionar(data: DataComemorativa) {
    ...
    startTransition(async () => {
      let res: { error?: string } | null | undefined = null;
      if (onAdicionar) {
        res = await onAdicionar(data);
      } else {
        ...
        res = await criarEntradaAction(fd);
      }
  ...
  return (
    <Card className="!border-royal-500/30 !bg-royal-500/5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {relevantes.map((d) => {
          const jaAdicionada =
            adicionadas.has(d.id) || (datasComEntrada?.includes(d.data) ?? false);
          return (
            <div
              key={d.id}
