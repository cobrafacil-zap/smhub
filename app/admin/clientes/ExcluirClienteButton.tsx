...
import { excluirClienteAction } from "@/lib/actions/cliente-convite-actions";
...
export function ExcluirClienteButton({ id, nome }: { id: string; nome: string }) {
  ...
  async function handleDelete() {
    ...
    // Sucesso: fecha modal, navega e força refresh da lista
    setOpen(false);
    setLoading(false);
      ...
      >
        <div className="p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-100">Excluir cliente?</h3>
              <p className="text-sm text-slate-400 mt-1">
                <strong className="text-slate-200">{nome}</strong> será removido permanentemente,
                junto com todos os planejamentos, relatórios, faturas e contratos vinculados.
